...
! function (e) {
    var i = 'style="width:{width};height:{height};"',
        ...
        h = function (i, t) {
            this.$element = e(i), u() ? (this.init(t), this.listen()) : this.$element.removeClass("file-loading")
        };
    h.prototype = {
        constructor: h,
        init: function (e) {
...
//                i.clear(!1);
        ...
        initPreview: function () {
            var i = this,
                t = "",
                n = i.initialPreview,
                a = i.initialPreviewCount,
                r = i.initialCaption.length,
                l = "preview-" + g(),
                o = r > 0 ? i.initialCaption : i.msgSelected.replace(/\{n\}/g, a),
                s = e(o).text();
            if (c(n) && a > 0) {
                for (var p = 0; a > p; p++) l += "-" + p, t += i.previewGenericTemplate.replace(/\{previewId\}/g, l).replace(/\{content\}/g, n[p]);
                a > 1 && 0 == r && (o = i.msgSelected.replace(/\{n\}/g, a))
            } else {
                if (!(a > 0)) return r > 0 ? (i.$caption.html(o), void i.$captionContainer.attr("title", s)) : void 0;
                for (var d = n.split(i.initialDelimiter), p = 0; a > p; p++) l += "-" + p, t += i.previewGenericTemplate.replace(/\{previewId\}/g, l).replace(/\{content\}/g, d[p]);
                a > 1 && 0 == r && (o = i.msgSelected.replace(/\{n\}/g, a))
            }
            i.initialPreviewContent = t, i.$preview.html(t), i.$caption.html(o), i.$captionContainer.attr("title", s), i.$container.removeClass("file-input-new")
        },
        ...
        clearFileInput: function () {
            var e = this,
                i = e.$element;
            /MSIE/.test(navigator.userAgent) ? (i.wrap("<form>").closest("form").trigger("reset"), i.unwrap()) : i.val("")
        },
        clear: function () {
            var e = this,
                i = arguments.length && arguments[0];
            if (i && i.preventDefault(), e.reader instanceof FileReader && e.reader.abort(), e.clearFileInput(), e.resetErrors(!0), i !== !1 && (e.$element.trigger("change"), e.$element.trigger("fileclear")), e.overwriteInitial && (e.initialPreviewCount = 0), e.overwriteInitial || d(e.initialPreviewContent)) {
                e.clearObjects(), e.$preview.html("");
                var t = !e.overwriteInitial && e.initialCaption.length > 0 ? e.original.caption : "";
                e.$caption.html(t), e.$captionContainer.attr("title", ""), e.$container.removeClass("file-input-new").addClass("file-input-new")
        ...
        parseFileType: function (e) {
            for (var i, t, n = 0; n < l.length; n++)
                if (cat = l[n], i = v(cat, self.fileTypeSettings) ? self.fileTypeSettings[cat] : p[cat], t = i(e.type, e.name) ? cat : "", "" != t) return t;
            return "other"
        },
        previewDefault: function (i, t) {
            var n = this,
                a = f.createObjectURL(i),
                r = e("#" + t),
                l = v("other", n.previewTemplates) ? n.previewTemplates.other : o.other;
        ...
        previewFile: function (e, i, t, n) {
            var n, a, r = this,
                ...
                u = (e.type, v(l, r.previewTemplates) ? r.previewTemplates[l] : o[l]),
                h = v(l, r.previewSettings) ? r.previewSettings[l] : s[l],
                b = parseInt(r.wrapTextLength),
                y = r.wrapIndicator,
                C = r.$preview,
                $ = c.indexOf(l) >= 0,
                x = d(m) || !d(m) && v(e.type, m);
            if ($ && x) {
                if ("text" == l) {
                    var F = w(i.target.result);
                    if (f.revokeObjectURL(n), F.length > b) {
                        var T = "text-" + g(),
                            I = .75 * window.innerHeight,
                            P = r.getLayoutTemplate("modal").replace(/\{id\}/g, T).replace(/\{title\}/g, p).replace(/\{height\}/g, I).replace(/\{body\}/g, F);
                        y = y.replace(/\{title\}/g, p).replace(/\{dialog\}/g, "$('#" + T + "').modal('show')"), F = F.substring(0, b - 1) + y
                    }
                    a = u.replace(/\{previewId\}/g, t).replace(/\{caption\}/g, p).replace(/\{type\}/g, e.type).replace(/\{width\}/g, h.width).replace(/\{height\}/g, h.height).replace(/\{data\}/g, F) + P
                } else a = u.replace(/\{previewId\}/g, t).replace(/\{caption\}/g, p).replace(/\{type\}/g, e.type).replace(/\{data\}/g, n).replace(/\{width\}/g, h.width).replace(/\{height\}/g, h.height);
                C.append("\n" + a)
            } else r.previewDefault(e, t)
        },
        readFiles: function (e) {
            function i(p) {
                if (p >= w) return l.removeClass("loading"), void o.html("");
                var v, g, y, C, $, x = u + "-" + p,
                    F = e[p],
                    T = t.slug(F.name),
                    I = (F.size ? F.size : 0) / 1e3,
                    P = f.createObjectURL(F),
                    E = 0,
                    L = t.allowedFileTypes,
                    S = d(L) ? "" : L.join(", "),
                    R = t.allowedFileExtensions,
                    j = d(R) ? "" : R.join(", "),
                    k = d(R) ? "" : new RegExp("\\.(" + R.join("|") + ")$", "i");
                if (I = I.toFixed(2), t.maxFileSize > 0 && I > t.maxFileSize) return y = t.msgSizeTooLarge.replace(/\{name\}/g, T).replace(/\{size\}/g, I).replace(/\{maxSize\}/g, t.maxFileSize), void(t.isError = t.showError(y, F, x, p));
                if (!d(L) && c(L)) {
                    for (g = 0; g < L.length; g++) C = L[g], v = h[C], $ = void 0 !== v && v(F.type, T), E += d($) ? 0 : $.length;
                    if (0 == E) return y = t.msgInvalidFileType.replace(/\{name\}/g, T).replace(/\{types\}/g, S), void(t.isError = t.showError(y, F, x, p))
                }
                return 0 != E || d(R) || !c(R) || d(k) || ($ = T.match(k), E += d($) ? 0 : $.length, 0 != E) ? t.showPreview ? void(a.length > 0 && "undefined" != typeof FileReader ? (o.html(s.replace(/\{index\}/g, p + 1).replace(/\{files\}/g, w)), l.addClass("loading"), r.onerror = function (e) {
                    t.errorHandler(e, T)
                }, r.onload = function (e) {
                    ...
                    setTimeout(function () {
                        ...
                        i(p + 1)
                    }, 1500), n.trigger("fileloaded", [F, x, p])
                }, r.onprogress = function (e) {
                    if (e.lengthComputable) {
                        var i = parseInt(e.loaded / e.total * 100, 10),
                            t = m.replace(/\{index\}/g, p + 1).replace(/\{files\}/g, w).replace(/\{percent\}/g, i).replace(/\{name\}/g, T);
                        setTimeout(function () {
                            o.html(t)
                        }, 1e3)
                    }
                }, b(F.type, T) ? r.readAsText(F) : r.readAsArrayBuffer(F)) : (t.previewDefault(F, x), n.trigger("fileloaded", [F, x, p]), setTimeout(i(p + 1), 1e3))) : void setTimeout(i(p + 1), 1e3) : (y = t.msgInvalidFileExtension.replace(/\{name\}/g, T).replace(/\{extensions\}/g, j), void(t.isError = t.showError(y, F, x, p)))
            }
            ...
            var t = this,
                ...
                s = t.msgLoading,
                m = t.msgProgress,
                u = (t.msgSelected, t.previewFileType, parseInt(t.wrapTextLength), t.wrapIndicator, "preview-" + g()),
                w = e.length,
                h = t.fileTypeSettings,
                b = v("text", h) ? h.text : p.text;
            i(0)
        },
        slug: function (e) {
        ...
        change: function (i) {
            ...
            var t, n = this,
                ...
                s = a.get(0).files,
                p = n.msgSelected,
                c = d(s) ? 1 : s.length + n.initialPreviewCount;
            
            // custom //
            c = 1;
            // ------- //

            if (n.hideFileIcon(), t = void 0 === i.target.files ? i.target && i.target.value ? [{
                    name: i.target.value.replace(/^.+\\/, "")
                }] : [] : i.target.files, 0 !== t.length) {
                n.resetErrors(), o.html(""), n.overwriteInitial || o.html(n.initialPreviewContent);
                var l = t.length;
                if (n.maxFileCount > 0 && l > n.maxFileCount) {
                    var v = n.msgFilesTooMany.replace(/\{m\}/g, n.maxFileCount).replace(/\{n\}/g, l);
                    return n.isError = n.showError(v, null, null, null), n.$captionContainer.find(".kv-caption-icon").hide(), n.$caption.html(n.msgValidationError), void n.$container.removeClass("file-input-new")
                }
                n.readFiles(s), n.reader = null;
                var m = c > 1 ? p.replace(/\{n\}/g, c) : r;
                n.isError ? (n.$captionContainer.find(".kv-caption-icon").hide(), m = n.msgValidationError) : n.showFileIcon(), n.$caption.html(m), n.$captionContainer.attr("title", e(m).text()), n.$container.removeClass("file-input-new");
                a.trigger("fileselect", [c, r])
            }
        },
        ...
        renderUpload: function () {
            var e = this,
                ...
                t = "",
                n = "";
            return e.showUpload ? (e.isDisabled && (n = " disabled "), t = d(e.uploadUrl) ? '<button type="submit" class="' + i + '"' + n + ">" + e.uploadIcon + e.uploadLabel + "</button>" : '<a href="' + e.uploadUrl + '" class="' + e.uploadClass + '"' + n + ">" + e.uploadIcon + e.uploadLabel + "</a>") : ""
        }
        ...
        if (u()) {
            ...
            return t.shift(), this.each(function () {
                var n = e(this),
                    a = n.data("fileinput"),
                    r = "object" == typeof i && i;
                a || n.data("fileinput", a = new h(this, e.extend({}, e.fn.fileinput.defaults, r, e(this).data()))), "string" == typeof i && a[i].apply(a, t)
            })
        }
